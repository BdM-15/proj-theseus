...
| 147 | `147-renderers-grounding-audit`   | 3b   | Verify ≥80% claim-to-citation ratio per skill                                                                                                                           | 🟡 active      | ⏳ pending    | Audit tool `tools/audit_skill_grounding.py` built (offline, reads `transcript.json`, splits final response into claims, marks each grounded if it cites a `chunk-xxxx` id, references a retrieved entity name, or is structural). Wired into `tests/skills/e2e/test_skills_smoke.py` behind `RUN_GROUNDING_AUDIT=1` (logs ratio per skill); `ENFORCE_GROUNDING_RATIO=1` flips sub-threshold to hard fail (default `MIN_GROUNDING_RATIO=0.80`). **Baseline (latest run per skill, all far below 0.80)**: competitive-intel 0.17, compliance-auditor 0.44, oci-sweeper 0.06, ot-prototype-strategist 0.04, price-to-win 0.00, proposal-generator 0.14, renderers 0.33 (utility-exempt at runtime), rfp-reverse-engineer 0.02, subcontractor-sow-builder 0.03. Next: tighten each SKILL.md citation rule. |
