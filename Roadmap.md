...
- **Documents page polish** — ✅ Addressed on `110-settings-documents-polish` (Apr 26). Status strip rebuilt as dashboard-style metric cards, pipeline state row given accent treatment, per-row status now a glowing cyberpunk pill, redundant Refresh removed. Remaining: detail drawer / advanced filters still pending.
...
- **Settings page substance** — ✅ Restructured as collapsible accordion with per-section neon stripes on `110-settings-documents-polish` (Apr 26). Editable `.env` knobs surfaced for query tuning; further `.env` exposure (`CHUNK_SIZE`, `CONTEXT_WINDOW`, model overrides) still pending with safe restart-on-save.

### Settings page — new features (added Apr 25)

- **Delete workspace feature.** ✅ Done on `108-settings-delete-workspace`. Settings → Danger Zone with per-workspace delete (granular Neo4j / rag_storage / inputs scopes + type-to-confirm) and a clean-slate Wipe-all panel. Reuses `tools/workspace_cleanup.py` helpers. **Follow-up**: no-active-workspace landing state so the active workspace itself becomes deletable without a prior switch — deferred (touches startup + lazy LightRAG init).
...
13. ~~**`108-settings-delete-workspace`**~~ ✅ Merged `9c5cc8d` (Apr 26). Settings → **Danger Zone** card lists every workspace under `rag_storage/`, the active Neo4j workspace labels, and `inputs/<ws>/` source folders with size/document-count metadata. Per-row delete modal with three independent scope checkboxes (Knowledge graph / Workspace storage / Source documents) plus an ALL-CAPS workspace-name confirmation (forces conscious differentiation from the actual lowercase name). A separate **Wipe ALL workspaces** panel performs a clean-slate reset (per-bucket selection + literal `DELETE ALL` confirmation) and self-restarts the server. New endpoints: `GET /api/ui/workspaces/inventory`, `POST /api/ui/workspaces/{name}/delete`, `POST /api/ui/workspaces/wipe-all`. All deletion paths reuse helpers from `tools/workspace_cleanup.py` — no duplicate implementations. The active workspace is server-side-protected (HTTP 409) and UI-disabled; users must switch first. Follow-up surfaced: a no-active-workspace landing state would let users delete the active workspace by dropping into a workspace picker rather than switching first — deferred (touches startup flow + LightRAG lazy-init).
14. ~~**`110-settings-documents-polish`**~~ ✅ Merged `c36abcc` (Apr 26). Settings page rebuilt as a collapsible accordion (`<details class="acc">`) with five distinct sections — **Workspace** (cyan), **Server Control** (amber), **Query Tuning** (cyan), **Models & Storage** (magenta), **Danger Zone** (red) — each rendering its own neon left-stripe via `details.acc::before` (avoids the Tailwind CDN `@apply` pseudo-element drop). Server Control adds **Clear LLM cache** (`POST /documents/clear_cache`), **Clear all documents** (`DELETE /documents`), and a soft **Restart server** action; runtime versions surface as accent pills (LightRAG / RAG-Anything / MinerU / Transformers). Documents page now mirrors the Dashboard metric-card vocabulary: 4-up status grid (`Processed` lime / `Processing` cyan / `Pending` amber / `Failed` red) using `card card-accent card-hover` with big `neon-text tabular-nums` numbers and corner `icon-tile`. Pipeline state row upgraded to a proper accent card (cyan idle / amber busy) with pulsing dot, dynamic state label, and truncated `latest_message` preview. Per-row status moved off the generic outlined pill onto a custom `.status-pill` (pure CSS to bypass Tailwind CDN `@apply` color resolution) — fully rounded capsule with per-status hex (`#00ff9c`/`#00f0ff`/`#ffb020`/`#fb7185`), `currentColor`-driven border + text-glow + outer halo, hover lift. Redundant Documents-page Refresh button removed (header global covers it). Endpoints discovered/wired: `DELETE /documents`, `POST /documents/cancel_pipeline`, `POST /documents/reprocess_failed`, `DELETE /documents/delete_document`. Single-file change to `src/ui/static/index.html`. Branch retained for history.
15. **`107-graph-quality-pass`** — _Next._ Tier 1A extraction prompt anchor discipline + Phase 2 normalization audit on `afcap6_drfp`. Re-extract and validate.
16. **Iterate** Tier 1A until `afcap6_drfp` matrix renders cleanly without UI tightening.
17. **Then** revisit issue #85 (RFP Intelligence overhaul) on top of clean data.
18. Polish backlog items (KG visualizer presets, Search page) interleaved between graph-quality re-runs.
