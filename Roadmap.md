...
13. ~~**`108-settings-delete-workspace`**~~ ✅ Merged (pending). Settings → **Danger Zone** card lists every workspace under `rag_storage/`, the active Neo4j workspace labels, and `inputs/<ws>/` source folders with size/document-count metadata. Per-row delete modal with three independent scope checkboxes (Knowledge graph / Workspace storage / Source documents) plus a type-the-workspace-name confirmation. A separate **Wipe ALL workspaces** panel performs a clean-slate reset (per-bucket selection + literal `DELETE ALL` confirmation) and self-restarts the server. New endpoints: `GET /api/ui/workspaces/inventory`, `POST /api/ui/workspaces/{name}/delete`, `POST /api/ui/workspaces/wipe-all`. All deletion paths reuse helpers from `tools/workspace_cleanup.py` (`_delete_neo4j_workspace`, `_delete_storage_workspace`, `_delete_inputs_workspace`) — no duplicate implementations. The active workspace is server-side-protected (HTTP 409) and UI-disabled; users must switch first. Follow-up surfaced: a no-active-workspace landing state would let users delete the active workspace by dropping into a workspace picker rather than switching first — deferred (touches startup flow + LightRAG lazy-init).
