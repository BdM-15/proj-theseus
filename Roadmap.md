...
14. **`110-settings-documents-polish`** — _Next._ Settings page button styling looks unfinished (Refresh / Wipe everything use raw bordered buttons; Server Control's `Restart Server` and the small action buttons feel inconsistent). Cluster Settings into collapsible sections (Server / Query Tuning / Workspace / Danger Zone) with `<details>`-style disclosures so the page doesn't dump every panel at once. Documents page has the same unpolished feel — empty states, action buttons, and per-row controls need the same vocabulary pass. Apply the chat-polish vocabulary (`btn-ghost`, `btn-primary`, neon accent borders, `glass-section-bar` headers, consistent icon + label spacing) and `lucide` icon sizing. Single branch.
15. **`107-graph-quality-pass`** (formerly Tier 1A) — extraction prompt anchor discipline + Phase 2 normalization audit on `afcap6_drfp`. Re-extract and validate.
