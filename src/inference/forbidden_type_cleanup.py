"""
Forbidden Type Cleanup - Phase 6 Semantic Post-Processing
==========================================================

Purpose: Eliminate UNKNOWN/other forbidden entity types using LLM retyping
Context: grok-4-fast-reasoning fails instruction compliance during extraction
...
- Batch processing (50 entities per LLM call for efficiency)
- Simple prompt: "Retype these entities using ONLY these 17 types"
- Cost: ~$0.005-0.01 per RFP (200 entities × 100 tokens = ~20K tokens)

Integration: Called from routes.py after RAG-Anything extraction, before relationship inference
"""
...
# Forbidden types that must be eliminated (89 total including case variants)
FORBIDDEN_TYPES = [
    "UNKNOWN",
    "unknown",
    "other",
    ...
    "control",
    "packaging",
    "service",  # NEW: The type that slipped through
    "review",
    "summary",
]


def identify_forbidden_entities(
    entities: List[Dict],
) -> List[Dict]:
    """
...
async def cleanup_forbidden_types(
    entities: List[Dict],
    llm_func: Callable[[str, str], Awaitable[str]],
    batch_size: int = 50,
) -> Tuple[List[Dict], Dict[str, str]]:
    """
    Main cleanup function: identifies and retypes forbidden entities.
    ...
    """
    logger.info("Starting forbidden type cleanup...")
    
    # Identify entities with forbidden types
    forbidden = identify_forbidden_entities(entities)
    
    if not forbidden:
        logger.info("✅ No forbidden types found - cleanup not needed")
        return entities, {}
    
    # Process in batches
    all_retypings = {}
    for i in range(0, len(forbidden), batch_size):
        batch = forbidden[i:i+batch_size]
        logger.info(f"Retyping batch {i//batch_size + 1} ({len(batch)} entities)...")
        
        batch_retypings = await retype_entities_batch(batch, llm_func)
        all_retypings.update(batch_retypings)
    ...
    logger.info(f"✅ Cleanup complete: {updated_count} entities retyped")
