...
class Neo4jGraphIO:
    ...
    def create_entities(self, entities: List[Dict]) -> int:
        ...
        Args:
            entities: List of entity dicts (from PydanticExtractor)
        
        Returns:
            Number of entities created
        """
        # Filter out any entities that might have slipped through with None names
        # Note: PydanticExtractor should have rescued these, but this is a final safety net.
        valid_entities = []
        for e in entities:
            if e.get('entity_name'):
                valid_entities.append(e)
            else:
                # This should theoretically never happen if PydanticExtractor is working
        ...
        SET n.entity_type = entity.entity_type,
            n.created_by = 'pydantic_extractor',
