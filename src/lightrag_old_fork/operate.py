...
# ============================================================================
# PHASE 3: GOVERNMENT CONTRACTING ONTOLOGY INTEGRATION
# ============================================================================
"""
Phase 3 Modifications:
1. Entity types injected via addon_params (see GovernmentRFPLightRAG wrapper)
2. Extraction prompts enhanced with ontology examples (see prompt.py)
3. Post-extraction validation ensures ontology compliance

Integration Points:
- Line ~2025: entity_types loaded from addon_params (already implemented)
- Line ~2069: Extraction prompts use ontology-enhanced PROMPTS (see prompt.py)
- Line ~950: _process_extraction_result filters invalid entities/relationships

For ontology validation, use GovernmentRFPLightRAG wrapper which:
- Pre-configures entity types via addon_params
- Post-validates extractions via OntologyInjector.validate_extraction()
- Generates quality gate reports via QualityAssuranceFramework
"""

# Optional: Import ontology for runtime validation (disabled by default to avoid deps)
try:
    from core.ontology import is_valid_relationship, EntityType
    ONTOLOGY_VALIDATION_AVAILABLE = True
    logger.info("✅ Ontology validation available for runtime checking")
except ImportError:
    ONTOLOGY_VALIDATION_AVAILABLE = False
    logger.debug("Ontology validation not available (use GovernmentRFPLightRAG wrapper)")
...
# ============================================================================
# PHASE 3: ONTOLOGY VALIDATION HELPERS (Optional Runtime Validation)
# ============================================================================

def _validate_entity_against_ontology(entity_data: dict, addon_params: dict) -> bool:
    """
    Optional runtime validation of extracted entity against ontology
    
    Args:
        entity_data: Entity dict with entity_name, entity_type, description
        addon_params: Global addon_params containing valid entity_types
    
    Returns:
        bool: True if valid, False if should be filtered
    
    Note: This is optional validation. Primary validation happens in
          GovernmentRFPLightRAG wrapper via OntologyInjector.
    """
    if not ONTOLOGY_VALIDATION_AVAILABLE:
        return True  # Skip validation if ontology not available
    
    entity_type = entity_data.get('entity_type', '').upper()
    valid_types = [et.upper() for et in addon_params.get('entity_types', [])]
    
    # Check if entity type is valid
    if valid_types and entity_type not in valid_types:
        logger.debug(
            f"Filtered entity '{entity_data.get('entity_name')}' with invalid type '{entity_type}'. "
            f"Valid types: {valid_types[:5]}..."
        )
        return False
...
def _validate_relationship_against_ontology(
    relationship_data: dict,
    entity_types_map: dict[str, str]
) -> bool:
    """
    Optional runtime validation of extracted relationship against ontology
    
    Args:
        relationship_data: Relationship dict with src_id, tgt_id, keywords, description
        entity_types_map: Map of entity names to their types
    ...
    # Get entity types for validation (if available)
    src_type = entity_types_map.get(src_id, 'UNKNOWN')
    tgt_type = entity_types_map.get(tgt_id, 'UNKNOWN')
    
    # Validate relationship pattern
    if src_type != 'UNKNOWN' and tgt_type != 'UNKNOWN':
        # Use ontology validation
        if not is_valid_relationship(src_type, keywords, tgt_type):
            logger.debug(
                f"Filtered relationship: {src_id}({src_type}) -{keywords}-> {tgt_id}({tgt_type})"
            )
            return False
...
async def extract_entities(
    ...
    # 🔥 ONTOLOGY INJECTION: Get ontology injector for prompt enhancement
    ontology_injector = global_config["addon_params"].get("ontology_injector", None)
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # 🔥 ONTOLOGY INJECTION: Enhance prompts with government contracting domain knowledge
        if ontology_injector is not None:
            # Enhance the system prompt with ontology guidance
            entity_extraction_system_prompt = ontology_injector.enhance_extraction_prompt(
                entity_extraction_system_prompt,
                section_context=None  # Could detect section from content in future
            )
            logger.info(f"🎯 Ontology-enhanced extraction prompt for chunk {chunk_key}")
    ...
    # 🔥 OLLAMA WORKER REFRESH: Track chunks processed for worker refresh
    # Match refresh interval to number of workers for optimal memory management
    chunks_since_refresh = 0
    refresh_interval = global_config.get("ollama_refresh_interval", chunk_max_async)  # Default to worker count
    
    async def _refresh_ollama_worker():
        """Refresh Ollama worker to prevent fatigue after processing multiple chunks."""
        try:
            # Force garbage collection to clear model context
            import gc
            gc.collect()
            
            # Small delay to allow Ollama to reset internal state
            await asyncio.sleep(0.5)
            
            logger.info(f"🔄 Ollama worker refreshed after {refresh_interval} chunks (total processed: {chunks_since_refresh})")
        except Exception as e:
            logger.warning(f"Worker refresh encountered issue (non-critical): {e}")

    async def _process_with_semaphore(chunk):
        nonlocal chunks_since_refresh
        async with semaphore:
            try:
                result = await _process_single_content(chunk)
                
                # 🔥 OLLAMA WORKER REFRESH: Check if we need to refresh
