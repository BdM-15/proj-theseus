...
to enable section-aware extraction for higher precision.
...
Reference: FAR 15.204-1, docs/CAPTURE_INTELLIGENCE_PATTERNS.md
"""
...
class UCFDetectionResult:
    ...
    Attributes:
        is_ucf: True if document follows UCF (confidence >= 0.70)
        confidence: Detection confidence score (0.0-1.0)
        detected_sections: List of section names found (e.g., ["Section L", "Section M"])
        missing_critical_sections: Critical sections not found
        section_pattern_type: "standard_ucf", "partial_ucf", "task_order", or "non_standard"
        reasons: Human-readable detection rationale
    """
    is_ucf: bool
    confidence: float
    detected_sections: List[str]
    missing_critical_sections: List[str]
    section_pattern_type: str
    reasons: List[str]


# UCF Section Patterns (FAR 15.204-1)
# Each section has multiple regex patterns for detection
UCF_SECTION_PATTERNS = {
    ...
    "Section J": [
        ...
        r"J-\d{7}",  # J-attachment numbering (7 digits)
        r"Annex(?:es)?",
    ],
    "Section L": [
        r"Section\s+L[:\s]",
...
# If document has L + M + supporting sections, likely UCF
CRITICAL_UCF_SECTIONS = ["Section L", "Section M"]
SUPPORTING_UCF_SECTIONS = ["Section H", "Section I", "Section J", "Section B"]

# Task Order / Non-Standard Patterns (indicates non-UCF format)
TASK_ORDER_PATTERNS = {
...
def detect_ucf_format(document_text: str, file_name: str = "") -> UCFDetectionResult:
    ...
    Threshold: is_ucf = True if confidence >= 0.70
    """
    detected_sections = []
    detection_reasons = []
    
    # Step 1: Check for standard UCF sections
    ...
    # Step 6: Filename heuristics (bonus)
    if file_name:
...
def extract_section_boundaries(document_text: str, detected_sections: List[str]) -> Dict[str, Tuple[int, int]]:
    ...
    Uses regex patterns to find section start positions, then determines
    end positions by finding the next section or end of document.
    ...
    for section in detected_sections:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                logger.debug(f"Section {section} boundaries: {start_offset}-{end_offset}")
                break
