...
async def main():
    ...
    logger.info(f"{YELLOW}2.{RESET} {BOLD}LightRAG Chunking{RESET} {CYAN}({chunk_size} tokens, {chunk_overlap} token overlap){RESET}")
    ...
    logger.info(f"   {CYAN}├─>{RESET} V2 modular prompts (~32K tokens, 23% reduction)")
    logger.info(f"   {CYAN}├─>{RESET} 23 annotated RFP examples inline with rules")
    logger.info(f"   {CYAN}├─>{RESET} Grok-4-fast-reasoning LLM")
    logger.info(f"   {CYAN}└─>{RESET} Semantic-first detection (UCF patterns)")
    logger.info("")
    logger.info(f"{YELLOW}4.{RESET} {BOLD}Relationship Extraction{RESET} {CYAN}(Main Processing){RESET}")
    logger.info(f"   {CYAN}├─>{RESET} 6 core patterns with confidence scoring")
    logger.info(f"   {CYAN}├─>{RESET} GUIDES (L↔M mapping), MEASURED_BY (QASP splits)")
    logger.info(f"   {CYAN}└─>{RESET} SATISFIED_BY/PRODUCES (deliverable traceability)")
    logger.info("")
    logger.info(f"{YELLOW}5.{RESET} {BOLD}Semantic Post-Processing{RESET} {GREEN}(Auto-triggered){RESET}")
    logger.info(f"   {CYAN}├─>{RESET} Cross-chunk relationship gap-filling")
    logger.info(f"   {CYAN}├─>{RESET} Orphan entity resolution")
    logger.info(f"   {CYAN}└─>{RESET} Validation & deduplication (trust but verify)")
    logger.info("")
