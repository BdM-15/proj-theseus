...
async def initialize_raganything():
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    # LightRAG Default Query Behavior (Restored)
    # ──────────────────────────────────────────────────────────────────────────────
    # REMOVED: Query-time PROMPTS modifications that deviated from LightRAG defaults.
    # Per LightRAG design principles, we now use the framework's native prompts for:
    # - keywords_extraction_examples: LightRAG's default examples
    # - rag_response: LightRAG's default response generation prompt
    #
    # This restoration improves query quality by:
    # 1. Using LightRAG's optimized keyword extraction (tuned by HKU team)
    # 2. Avoiding prompt bloat that fragments hybrid retrieval
    # 3. Maintaining consistency with RAG-Anything's design intent
    #
    # Domain-specific guidance is now applied only at extraction time via:
    ...
    logger.info("✅ Using LightRAG default query prompts (no query-time PROMPTS modifications)")
