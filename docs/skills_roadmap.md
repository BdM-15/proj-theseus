...
| **4**   | External datasources (SAM.gov, USAspending, FPDS)  | 🟡 4a+4b+4c done       | `ecbcc29` (client), `1d3df18` (USAspending vendored). 4d–4f pending.        |
...
| **4d**    | Promote `competitive-intel` skill from PLACEHOLDER → real tools-mode skill: declare `metadata.mcps: [usaspending]`, rewrite SKILL.md body checklist to call USAspending tools through the runtime, ship `intel_award_history(naics=…)` end-to-end via the agent loop. Closes the 4c smoke-test goal at the skill layer (4c proved the transport; 4d proves a skill consumes it).                                                                | ⏳ Not started |
...
| **4f**    | Vendor remaining MCPs from `1102tools/federal-contracting-mcps`: `sam_gov` (key-gated), `fpds`, `bls_oews`, `gsa_calc`, `gsa_perdiem`. `federal_register`, `ecfr`, `regulations_gov` deferred until first skill needs them. Each gets a UPSTREAM.md with attribution + re-vendor instructions. Same `uvx` + manifest pattern as 4c.                                                                                                             | ⏳ Not started |
