...
| **6a-2**  | Inline preview pane per format: native PDF (iframe), Mammoth.js (DOCX, lazy-loaded), SheetJS (XLSX, lazy-loaded with sheet tabs), `<video>` (MP4/WEBM), native `<img>` (PNG/JPG/GIF/SVG), `<pre>` (MD/JSON/CSV/TXT)                                                    | ✅ Done — `140-phase-6a2-inline-preview` (right-side slide-in drawer, reuses existing artifact endpoint)                                 |
| **6a-2**  | Per-artifact extras: Pin (anchor-to-top, persisted via localStorage); Regenerate + Archive deferred to a follow-up                                                                                                                                                    | ✅ Done (Pin) — Lucide `anchor` icon (Ship-of-Theseus mythos), cyan glow active state, pinned rows sort to top; Regenerate/Archive ⏳   |
| **6a-2**  | KG back-links — each artifact's JSON envelope is parsed for `chunk-<hex>` ids; chips above the body open the Phase 6b.2 chunk-preview modal                                                                                                                           | ✅ Done — `140-phase-6a2-inline-preview` (closes the artifact → chunk → entity audit chain in two clicks)                                |
