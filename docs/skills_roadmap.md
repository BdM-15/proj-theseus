...
| **6b.2**  | Citation chain surfaces inline: every `chunk_id` referenced in the artifact resolves to the originating chunk's preview                                                 | 🟡 Partial — chunk-id chips with copy-to-clipboard render in 6b.1; inline chunk preview modal deferred (needs new `GET /api/ui/chunks/{chunk_id}`) |
